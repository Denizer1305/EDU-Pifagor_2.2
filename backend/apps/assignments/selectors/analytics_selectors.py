...
def get_assignment_statistics(*, assignment):
    ...
    return {
        ...
        "failed_count": submissions.filter(passed=False).exclude(
...
def get_student_assignment_progress(*, student, assignment):
    ...
    return {
        ...
        "last_submission_attempt_number": last_submission.attempt_number if last_submission else 0,
        "last_submission_score": _num(last_submission.final_score) if last_submission else 0,
        "last_submission_percentage": _num(last_submission.percentage) if last_submission else 0,
