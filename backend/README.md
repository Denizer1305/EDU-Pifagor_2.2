...
- production deploy check, coverage threshold и security audit.
...
Он выполняет основной backend pipeline:
...
`make check-prod` запускает Django production deploy check:

```bash
python manage.py check --deploy --settings=config.settings.prod
```

Для этой команды в `Makefile` используются безопасные CI-placeholder значения окружения. Настоящие production-секреты в репозиторий не добавляются.
...
Для чувствительных auth endpoints используется scoped throttling DRF:
...
Тесты проверяют конфигурацию throttling на уровне view-классов: наличие `ScopedRateThrottle` и корректного `throttle_scope`.
...
- часть `APIView` endpoints требует `serializer_class` или `@extend_schema`;
- warnings по enum naming и operationId collisions вынесены в backlog;
- OpenAPI cleanup планируется отдельным этапом.
